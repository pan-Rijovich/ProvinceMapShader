...
public class BorderShower : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (var br in _borders.Values) 
        {
            foreach (var b in br) 
            {
                ...
                { 
                    int posUp = b[i].position + _width;
                    int posDown = b[i].position - _width;
                    int posLeft = b[i].position - 1;
    ...
    private void DrawLines(uint color)
    {
        foreach (var a in _provincesBorders[color])
        {
            foreach (var b in _borders[a])
            {
                ...
                _lineRenderer.Last().positionCount = 50;
                _lineRenderer.Last().SetPositions(BSpline.CalculateBSpline(points.ToList(), 50).ToArray());
            }
        }
    }
